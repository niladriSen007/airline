...
interface EditCityProps {
  ...
  onSave: (data: CityData) => void;
}

export const EditCity: React.FC<EditCityProps> = ({
  isOpen,
  onClose,
  initialData,
  onSave,
}) => {
  const [formData, setFormData] = useState<CityData>(initialData);
          ...
          <div className="relative w-full max-w-180 z-101 my-auto pointer-events-none">
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-4 md:px-8">
                <h2 className="text-[18px] font-semibold text-[#010101] dark:text-white">
                  Edit your profile
                </h2>
                <button
                  title="close"
                  onClick={onClose}
                  className="text-[#a0a0a0] hover:text-gray-400 transition-colors p-1"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              {/* Body */}
              <div
                className="flex flex-col md:flex-row border-t-[1.6px] border-b-[1.6px] rounded-[18px] 
              ...
              >
                ...
                <div className="flex-1 p-6 space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[14px] font-medium text-[#706f6f] dark:text-[#A1A1A6]">
                      Full name
                    </label>
                    <input
                      title="fullname"
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1 space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <ChevronDown
                          size={20}
                          className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[#131313]/40 dark:text-gray-500"
                        />
                      </div>
                    </div>
                    <div className="flex-1 space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <Clock
                          size={18}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-[#131313]/40 dark:text-gray-500"
                        />
                      </div>
                    </div>
                  </div>
                ...
                {/* <div className="flex-1 p-8 px-6 flex flex-col items-center justify-center">
                  <span className="text-[14px] font-medium mb-4 text-[#706f6f] dark:text-[#A1A1A6]">
                    Preview
                  </span>
                  ...
                  <h3 className="text-[18px] font-bold text-[#101010] dark:text-white text-center">
                    {formData.fullName}
                  </h3>
                  <p className="text-[14px] mb-4 text-[#777678] dark:text-[#A1A1A6] text-center">
                    {formData.title}
                  </p>
                  <div
                    className="flex items-center gap-2 px-3 shadow-sm py-1 rounded-full text-[12px] font-medium 
                                                 bg-[#F7F7F9] text-[#101010]/60 
                                                 dark:bg-[#3A3A3C] dark:text-[#A1A1A6]"
                  >
                    <Clock
                      size={12}
                      className="text-[#777678] dark:text-[#A1A1A6]"
                    />
