...
const Navbar = () => {
  ...
  return (
    <div className="relative">
      <header className="bg-transparent border-b-2 shadow-sm backdrop-blur-sm dark:bg-zinc-950/50 sticky top-0 z-50 transition-all duration-500">
        <div className="max-w-7xl mx-auto w-full px-4 py-4 sm:px-6">
          <nav
            className={cn(
              ...
              sticky
                ? "p-2.5 bg-white backdrop-blur-lg border border-border/40 shadow-2xl shadow-primary/5 rounded-full"
                : "bg-transparent border-transparent",
            )}
          >
            <div className="flex items-center gap-3.5 lg:gap-2">
              <HugeiconsIcon icon={PlaneIcon} className="text-lime-500" />
              <span className="text-xl font-bold text-lime-500 hidden lg:block">
                Airline
              </span>
            </div>
            <div>
              <NavigationMenu className="max-lg:hidden bg-transparent border border-zinc-700/50 px-2 py-1 rounded-full">
                <NavigationMenuList className="flex gap-0">
                  {navigationData.map((navItem) => (
                    <NavigationMenuItem key={navItem.title}>
                      <NavigationMenuLink
                        href={navItem.href}
                        className="px-2 lg:px-4 py-2 text-sm font-medium rounded-full text-black dark:text-white hover:text-foreground hover:bg-gray-400/10 outline outline-transparent hover:outline-border hover:shadow-xs transition tracking-normal"
                      >
                        {navItem.title}
                      </NavigationMenuLink>
                    </NavigationMenuItem>
